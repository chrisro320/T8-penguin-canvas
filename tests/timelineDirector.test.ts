...
test('timeline director compiles shot names durations prompts and @ image refs into Jimeng request', () => {
  ...
  assert.equal(request.duration, 10.3);
...
test('timeline director enforces total duration between 4 and 15 seconds while editing', () => {
  ...
  assert.ok(tooLong.slice(0, -1).every((block) => block.durationSec >= 1 && block.durationSec <= 12));
...
test('timeline director frontend exposes storyboard-aligned controls without shot override and bridge clutter', () => {
  ...
  assert.match(node, /images:\s*dedupe\(\[\.\.\.requestImages,\s*\.\.\.timelineImageUrls\]\)/);
  assert.match(node, /videos:\s*requestVideos/);
  assert.match(node, /audios:\s*requestAudios/);
  assert.match(node, /referenceTexts:\s*\[compiledPrompt\]/);
  assert.match(node, /Codex Agent 优化/);
  ...
  assert.match(node, /grid-cols-\[minmax\(0,1fr\)_minmax\(0,1fr\)\]/);
